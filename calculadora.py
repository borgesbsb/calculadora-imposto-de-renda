...
class Calculadora:
    def __init__(self, csv_file):
        ...
        self.carteira = pd.DataFrame(columns=['Data', 'Nome','Preço', 'Preco Medio','Quantidade','Quantidade Media', 'Oper', 'Resultado Auferido', 'Lucro/Prejuizo'])
    ...
    def calcular_carteira(self):
        ...
        for index, row in self.df.iterrows():
            ...
            if self.carteira.loc[self.carteira['Nome'] == acao].empty:
                ...
                self.carteira.loc[index] = [data_operacao, acao, preco, preco_medio,quantidade, quantidade, operacao, 0, self.lucro_prejuizo]
            else:
                preco_medio_tmp = self.carteira.loc[self.carteira['Nome'] == acao, 'Preco Medio'].values[-1]
                quantidade_media_tmp = self.carteira.loc[self.carteira['Nome'] == acao, 'Quantidade Media'].values[-1]
                resultado_auferido_temp = self.carteira.loc[self.carteira['Nome'] == acao, 'Resultado Auferido'].values[-1]
                ...
                if operacao == 'Compra':
                    ...
                    self.carteira.loc[index] = [data_operacao, acao, preco, preco_medio,quantidade, quantidade_media, operacao, resultado_auferido_temp, self.lucro_prejuizo]
                else:
                    ...
                    self.carteira.loc[index] = [data_operacao, acao,preco, preco_medio,quantidade, quantidade_media, operacao, resultado_auferido, self.lucro_prejuizo - imposto]
